...
// Simple auth for PUT & DELETE
async function authenticate(req) {
  const token = req.headers.get("authorization")?.split(" ")[1];
  return token === process.env.ADMIN_TOKEN;
}

// Build query for both ObjectId and string _id
function buildIdQuery(id) {
  if (ObjectId.isValid(id)) return { $or: [{ _id: new ObjectId(id) }, { _id: id }] };
  return { _id: id };
}
...
export async function PUT(req, { params }) {
  ...
  try {
    ...
    return new Response(
      JSON.stringify({ message: "Product updated", modifiedCount: result.modifiedCount }),
      { status: 200 }
    );
  } catch (err) {
...
export async function DELETE(req, { params }) {
  ...
  try {
    ...
    return new Response(
      JSON.stringify({ message: "Product deleted", deletedCount: result.deletedCount }),
      { status: 200 }
    );
  } catch (err) {
